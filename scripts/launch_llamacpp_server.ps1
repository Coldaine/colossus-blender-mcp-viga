...
if ($llamaServer) {
  ...
  & $llamaServer.Source --model "$ModelPath" --host $ListenAddress --port $Port
  exit 0
}
...
Write-Host ("Endpoint: http://{0}:{1}/v1" -f $ListenAddress, $Port) -ForegroundColor Cyan

# llama-cpp-python provides an OpenAI-compatible server module in most builds.
& $python -m llama_cpp.server --model "$ModelPath" --host $ListenAddress --port $Port
